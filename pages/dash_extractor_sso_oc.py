...
def _listar_archivos_diario():
    res = sorted([p.name for p in sso.CARPETA_DIARIO.glob("*_RESUMEN.csv")])
    det = sorted([p.name for p in sso.CARPETA_DIARIO.glob("*_DETALLES.csv")])
    return res, det


@st.cache_data(ttl=60, show_spinner=False)
def _listar_maestros():
    ruta_res = sso.CARPETA_MAESTROS / "OC_Maestro_Resumen.csv"
    ruta_det = sso.CARPETA_MAESTROS / "OC_Maestro_Detalles.csv"
    return ruta_res.exists(), ruta_det.exists(), ruta_res, ruta_det


@st.cache_data(ttl=600, show_spinner=False)
def _cargar_maestro_resumen():
    ruta = sso.CARPETA_MAESTROS / "OC_Maestro_Resumen.csv"
    if not ruta.exists():
        return pd.DataFrame()
...
def _cargar_maestro_detalles():
    ruta = sso.CARPETA_MAESTROS / "OC_Maestro_Detalles.csv"
    if not ruta.exists():
        return pd.DataFrame()
    return pd.read_csv(ruta, sep=";", encoding="utf-8-sig", dtype=str)
...
st.markdown(
    ...
    <div style="padding: 1.2rem 1.5rem; margin-bottom: 1.5rem; background: linear-gradient(90deg, #138AEC, #3E9FEF); color: white; border-radius: 14px; box-shadow: 0 4px 10px rgba(0,0,0,0.15);">
        <div style="font-size: 28px; font-weight: 800;">🧩 Extractor SSO - Órdenes de Compra</div>
                ...
                with st.spinner("Ejecutando..."):
                    if accion == "Descargar rango":
                        ...
                        def _cb_progress(codigo: str, done: int, total: int) -> None:
                            ...
                            oc_txt.markdown(f"**OC actual:** `{codigo}`")
                            pct = int(done / total * 100) if total else 0
                            barra.progress(pct)

                        while cur <= f_fin:
                            progreso_txt.markdown(f"**Día:** `{cur}`")
                            oc_txt.markdown("**OC actual:** iniciando...")
                            barra.progress(0)
...
with tab_resultados:
    ...
    if not df_res.empty and "FechaCreacion" in df_res.columns:
        ...
        if not df_res_vis.empty:
            ...
            fig = px.bar(serie, x="Mes", y="Cantidad", title="Órdenes por mes")
    ...
    if not df_res.empty:
        ...
        st.download_button(
            ...
            data=csv_bytes,
            file_name="OC_Maestro_Resumen.csv",
            mime="text/csv",
        )
    ...
    if not df_det.empty:
        ...
        st.download_button(
            ...
            data=csv_bytes,
            file_name="OC_Maestro_Detalles.csv",
            mime="text/csv",
        )
