...
st.markdown("## 📋 Detalles de Compradores")
# Pestañas para organizar la visualización
tab1, tab2, tab3  = st.tabs(["📊 General", " 📊 Estadísticas"," 🔍 Búsqueda"])
...
with tab3:
    ...
    # 🔍 BUSCADOR UNIVERSAL
    # ==========================================================
    # Colocamos el buscador antes de dividir la tabla para que filtre ambas categorías
    texto_busqueda = st.text_input(
        "🔍 Buscar en tablas FSC:", 
        placeholder="Escribe ID, nombre, unidad o cualquier dato...",
        help="Este buscador filtrará las filas que contengan el texto en CUALQUIER columna."
    )

    if texto_busqueda:
        # 1. Convertimos todo a string para poder buscar
        # 2. Verificamos si alguna columna contiene el texto (case=False ignora mayúsculas)
        # 3. .any(axis=1) devuelve True si al menos una columna cumple la condición
        mascara_busqueda = tabla_base.astype(str).apply(
            lambda x: x.str.contains(texto_busqueda, case=False, na=False)
        ).any(axis=1)
        
        # Filtramos la tabla base
        tabla_base = tabla_base[mascara_busqueda]
    ...
    with st.expander(f"⚠️ Ver FSC Pendientes -   {len(tabla_pendientes):,} registros pendientes.", expanded=False):
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "UnionMP": st.column_config.TextColumn("UnionMP", pinned=True),
                "fecha derivado": st.column_config.DateColumn("Fecha derivado", format="DD-MM-YYYY"),
                "monto estimado": st.column_config.NumberColumn("Monto estimado (CLP)", format="$ %,.0f")
            }
        )
